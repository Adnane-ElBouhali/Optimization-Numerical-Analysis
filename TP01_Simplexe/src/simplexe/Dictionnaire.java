...
public class Dictionnaire extends Observable {
	...
	public void pivoter(int iS, int jE) {
		int numRows = D.length;
	    int numCols = D[0].length;

	    // Étape 1 : Mettre à jour la ligne pivot (iS)
	    double pivotValue = D[iS][jE];
	    for (int j = 0; j < numCols; j++) {
	        D[iS][j] /= pivotValue;
	    }

	    // Étape 2 : Mettre à jour les autres lignes
	    for (int i = 0; i < numRows; i++) {
	        if (i != iS) {
	            double factor = D[i][jE];
	            for (int j = 0; j < numCols; j++) {
	                D[i][j] -= factor * D[iS][j];
	            }
	        }
	    ...
	    // Étape 3 : Mettre à jour les indices des variables entrantes et sortantes
	    // Assurez-vous de réorganiser les indices correctement

	    // Étape 4 : Réarranger l'ordre des lignes pour conserver l'ordre
	    // Supprimez d'abord la ligne pivot
	    double[][] temp = new double[numRows - 1][numCols];
	    int destRow = 0;
	    for (int srcRow = 0; srcRow < numRows; srcRow++) {
	        if (srcRow != iS) {
	            temp[destRow++] = D[srcRow];
	        }
	    }
	    D = temp;

	    // Ajoutez la ligne pivot à la fin du dictionnaire
	    D = Arrays.copyOf(D, numRows);
	    D[numRows - 1] = new double[numCols];
	    for (int j = 0; j < numCols; j++) {
	        D[numRows - 1][j] = 0.0;
	    }
